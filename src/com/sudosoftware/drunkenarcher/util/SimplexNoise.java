...
public class SimplexNoise {
	public static double calculateNoiseHeight(int x, int z, double startFrequency, int octaveCount, double persistence) {
		double noise = 0;
		double normalizeFactor = 0;

		// Number of cycles per unit length that a specific coherent-noise
		// function outputs.
		double frequency = startFrequency;

		// Max value returned from this function.
		double amplitude = 1;

		for (int i = 0; i < octaveCount; i++) {
			normalizeFactor += amplitude;

			// Sum up the simplex noises.
			noise += amplitude * SimplexNoise.noise(frequency * x, frequency * z);

			// Double the frequency to calculate next octave.
			frequency *= 2;

			// Persistence is a multiplier that determines how quickly
			// the amplitudes diminish for a successive octave.
